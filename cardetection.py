...
def yolo_filter_boxes(box_confidence, boxes, box_class_probs, threshold = .6):
    ...
    Returns:
    scores -- tensor of shape (None,), containing the class 
        probability score for selected boxes
    ...
    Note: "None" is here because you don't know the exact number of 
        selected boxes, as it depends on the threshold. 
    ...
    # Step 3: Create a filtering mask based on "box_class_scores" 
    # by using "threshold". The mask should have the
...
def yolo_non_max_suppression(scores, boxes, classes, max_boxes = 10, iou_threshold = 0.5):
    ...
    Note: The "None" dimension of the output tensors has obviously to be 
        less than max_boxes. Note also that this
...
#0. creating a session to start your graph
sess = K.get_session()

#0. Defining classes, anchors and image shape.
class_names = read_classes("model_data/coco_classes.txt")
anchors = read_anchors("model_data/yolo_anchors.txt")
image_shape = (720., 1280.)  

#1. Loading a pretrained model - yolo_model.input is given to yolo_model. The model is used to compute the output yolo_model.output
yolo_model = load_model("model_data/yolo.h5")
#yolo_model.summary()

#2. Convert output of the model to usable bounding box tensors - yolo_model.output is processed by yolo_head. It gives you yolo_outputs
yolo_outputs = yolo_head(yolo_model.output, anchors, len(class_names))

#3. Filtering boxes - yolo_outputs goes through a filtering function, yolo_eval. It outputs your predictions: scores, boxes, classes
